...
<blockquote>AKSES ALL<blockquote>
/update`;
...
//======CASE BUG 6======\\
bot.onText(/\/XqlNoClick(?:\s+(\d+))?/i, async (msg, match) => {
  try {
    const chatId = msg.chat.id
    const senderId = msg.from.id
    const username = msg.from.username ? `@${msg.from.username}` : msg.from.first_name || "User"
...
//======CASE BUG 11======\\
bot.onText(/\/ForceUix(?:\s+(\d+))?/i, async (msg, match) => {
  try {
    const chatId = msg.chat.id
    const senderId = msg.from.id
    const username = msg.from.username ? `@${msg.from.username}` : msg.from.first_name || "User"
...
//======CASE BUG 12======\\
bot.onText(/\/Xios(?:\s+(\d+))?/i, async (msg, match) => {
  try {
    const chatId = msg.chat.id
    const senderId = msg.from.id
    const username = msg.from.username ? `@${msg.from.username}` : msg.from.first_name || "User"
